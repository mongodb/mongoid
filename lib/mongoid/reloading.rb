...
module Mongoid #:nodoc:
  ...
  module Reloading
    ...
    def reload_root_document(scoped = true)
      {}.merge(collection.find_one({_id: id}, default_scoped_fields(scoped)) || {})
    end

    def default_scoped_fields(scoped) #:nodoc:
      return {} if !scoped || !self.class.default_scoping
      (self.class.default_scoping.call.options || {}).select{|key, _| key == :fields }
    end
