      ...
      class HasManyThrough
        ...
        class Eager < Association::Eager
          ...
          def preload
            ...
            targets_by_owner_fk = build_targets_map(intermediates, through_fk, source_assoc, owner_pk)
          ...
          def build_targets_map(intermediates, through_fk, source_assoc, owner_pk)
            if source_assoc.stores_foreign_key?
              fk_on_intermediate_targets_map(intermediates, through_fk, source_assoc)
            else
              fk_on_source_targets_map(intermediates, through_fk, source_assoc, owner_pk)
            end
          end
          ...
          def fk_on_source_targets_map(intermediates, through_fk, source_assoc, owner_pk)
            intermediate_pks = intermediates.filter_map { |i| i.public_send(owner_pk) }.uniq
            targets = source_assoc.klass.where(
            ...
            intermediates.each do |i|
              owner_fk_val = i.public_send(through_fk)
              matched = targets_by_source_fk[i.public_send(owner_pk)] || []
              result[owner_fk_val].concat(matched)
            end
            result
          end
