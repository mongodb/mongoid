...
describe 'Queries with Mongoid::RawValue criteria' do
  ...
  let!(:band1) do
    Band.create!(name: '1', likes: 0, rating: 0.9, sales: BigDecimal('90'), decibels: 20..80, founded: today,
                 updated: now_utc)
  end
  let!(:band2) do
    Band.create!(name: '2', likes: 1, rating: 1.0, sales: BigDecimal('100'), decibels: 30..90, founded: today,
                 updated: now_utc + 1.days)
  end
  let!(:band3) do
    Band.create!(name: '3', likes: 1, rating: 2.2, sales: BigDecimal('220'), decibels: 40..100, founded: today + 1.days,
                 updated: now_utc + 2.days)
  end
  let!(:band4) do
    Band.create!(name: '3', likes: 2, rating: 3.1, sales: BigDecimal('310'), decibels: 50..120, founded: today + 1.days,
                 updated: now_utc + 3.days)
  end
  let!(:band5) do
    Band.create!(name: '4', likes: 3, rating: 3.1, sales: BigDecimal('310'), decibels: 60..150, founded: today + 2.days,
                 updated: now_utc + 3.days, labels: labels)
  end

  let!(:band6) do
    id = BSON::ObjectId.new
    Band.collection.insert_one(_id: id, name: 1, likes: '1', rating: '3.1', sales: '310', decibels: '90',
                               founded: '2020-01-02', updated: '2020-01-04 16:00:00 UTC')
    Band.find(id)
  end

  let!(:band7) do
    id = BSON::ObjectId.new
    Band.collection.insert_one(_id: id, name: 1.0, decibels: 90.0, founded: 1_577_923_200, updated: 1_578_153_600)
    Band.find(id)
  end
  ...
  context 'Mongoid::RawValue<Integer>' do
    ...
    context 'Date field' do
      it 'does not match objects with raw value' do
        expect(Band.where(founded: Mongoid::RawValue(1_577_923_200)).to_a).to eq [ band7 ]
      end

      it 'matches objects without raw value' do
        expect(Band.where(founded: 1_577_923_200).to_a).to eq [ band3, band4 ]
      end
    end

    context 'Time field' do
      it 'does not match objects with raw value' do
        expect(Band.where(updated: Mongoid::RawValue(1_578_153_600)).to_a).to eq [ band7 ]
      end

      it 'matches objects without raw value' do
        expect(Band.where(updated: 1_578_153_600).to_a).to eq [ band4, band5 ]
      end
    end
  end
  ...
  context 'Mongoid::RawValue<Float>' do
    ...
    context 'Date field' do
      it 'does not match objects with raw value' do
        expect(Band.where(founded: Mongoid::RawValue(1_577_923_200.0)).to_a).to eq [ band7 ]
      end

      it 'matches objects without raw value' do
        expect(Band.where(founded: 1_577_923_200.0).to_a).to eq [ band3, band4 ]
      end
    end

    context 'Time field' do
      it 'does not match objects with raw value' do
        expect(Band.where(updated: Mongoid::RawValue(1_578_153_600.0)).to_a).to eq [ band7 ]
      end

      it 'matches objects without raw value' do
        expect(Band.where(updated: 1_578_153_600.0).to_a).to eq [ band4, band5 ]
      end
    end
  end
  ...
  context 'Mongoid::RawValue<BigDecimal>' do
    ...
    context 'Float field' do
      ...
      it 'fuzzy matches objects with raw value' do
        expect(Band.gte(rating: Mongoid::RawValue(BigDecimal('3.099'))).lte(rating: Mongoid::RawValue(BigDecimal('3.101'))).to_a).to eq [
          band4, band5
        ]
      end
  ...
  context 'Mongoid::RawValue<Range>' do
    ...
    context 'Date field' do
      it 'raises a BSON error with raw value' do
        expect { Band.where(founded: Mongoid::RawValue(1_577_923_199..1_577_923_201)).to_a }.to raise_error BSON::Error::UnserializableClass
      end

      it 'matches objects without raw value' do
        expect(Band.where(founded: 1_577_923_199..1_577_923_201).to_a).to eq [ band1, band2, band3, band4 ]
      end
    end

    context 'Time field' do
      it 'raises a BSON error with raw value' do
        expect { Band.where(founded: Mongoid::RawValue(1_578_153_599..1_578_153_600)).to_a }.to raise_error BSON::Error::UnserializableClass
      end

      it 'matches objects without raw value' do
        expect(Band.where(updated: 1_578_153_599..1_578_153_600).to_a).to eq [ band4, band5 ]
      end
    end
  end
  ...
  context 'Mongoid::RawValue<Time>' do
    let!(:band7) do
      id = BSON::ObjectId.new
      Band.collection.insert_one(_id: id, name: Time.at(1), likes: Time.at(1), rating: Time.at(3.1),
                                 sales: Time.at(310), decibels: Time.at(90))
      Band.find(id)
    end
    ...
    context 'Date field' do
      it 'matches objects with raw value when exact' do
        expect(Band.where(founded: Mongoid::RawValue(Time.at(1_577_923_200))).to_a).to eq [ band3, band4 ]
      end

      it 'does not match objects with raw value when non-exact' do
        expect(Band.where(founded: Mongoid::RawValue(Time.at(1_577_923_199))).to_a).to eq []
      end

      it 'matches objects without raw value when exact' do
        expect(Band.where(founded: Time.at(1_577_923_200).utc).to_a).to eq [ band3, band4 ]
      end

      it 'matches objects without raw value when 1 second before midnight' do
        expect(Band.where(founded: Time.at(1_577_923_199).utc).to_a).to eq [ band1, band2 ]
      end

      it 'matches objects without raw value when 1 second after midnight' do
        expect(Band.where(founded: Time.at(1_577_923_201).utc).to_a).to eq [ band3, band4 ]
      end
    end

    context 'Time field' do
      it 'matches objects with raw value' do
        expect(Band.where(updated: Mongoid::RawValue(Time.at(1_578_153_600))).to_a).to eq [ band4, band5 ]
      end

      it 'matches objects without raw value' do
        expect(Band.where(updated: Time.at(1_578_153_600)).to_a).to eq [ band4, band5 ]
      end
    end
  end

  context 'Mongoid::RawValue<Date>' do
    let!(:band7) { Band.create!(updated: Time.at(1_577_923_200)) }

    context 'Date field' do
      it 'matches objects with raw value' do
        expect(Band.where(founded: Mongoid::RawValue(Time.at(1_577_923_200).utc.to_date)).to_a).to eq [ band3, band4 ]
      end

      it 'matches objects without raw value when non-exact' do
        expect(Band.where(founded: Time.at(1_577_923_200).utc.to_date).to_a).to eq [ band3, band4 ]
      end
    end

    context 'Time field' do
      it 'matches objects with raw value' do
        expect(Band.where(updated: Mongoid::RawValue(Time.at(1_577_923_200).utc.to_date)).to_a).to eq [ band7 ]
      end

      it 'matches objects without raw value' do
        Time.use_zone('UTC') do
          expect(Band.where(updated: Time.at(1_577_923_200).utc.to_date).to_a).to eq [ band7 ]
        end
      end

      it 'does not matches objects without raw value when in other timezone' do
        Time.use_zone('Asia/Tokyo') do
          expect(Band.where(updated: Time.at(1_577_923_200).utc.to_date).to_a).to eq []
        end
      end
    end
  end
