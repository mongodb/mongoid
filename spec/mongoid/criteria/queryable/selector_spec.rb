...
describe Mongoid::Criteria::Queryable::Selector do
  ...
  describe '#__deep_copy__' do
    ...
    it 'performs a deep copy' do
      expect(cloned['field']).not_to equal(selection)
    end

    it 'clones n levels deep' do
      expect(cloned['field']['$in']).not_to equal(value)
    end
  end
      ...
      context 'when no serializers are provided' do
        ...
        context 'when providing a Mongoid::RawValue' do
          ...
          context 'and raw_value is a complex type' do
            ...
            before do
              selector.send(method, 'key',
                            Mongoid::RawValue([ { foo: 1, 'Bar' => [ /baz/, BigDecimal('2'), 4..5 ] }, 3 ]))
            end
