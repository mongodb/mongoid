...
describe Mongoid::Association::Embedded::EmbedsMany::Proxy do
    ...
    describe "##{method}" do
      context 'when the parent is a new record' do
        ...
        it 'does not save the new document' do
          expect(address).not_to be_persisted
        end
        ...
        context 'with a limiting default scope' do
          ...
          context 'when the document does not match the scope' do
            ...
            it "doesn't append to the target" do
              expect(person.appointments._target).not_to eq([ inactive ])
            end
      ...
      context 'when the parent and child have a cyclic relation' do
        context 'when the parent is a new record' do
          ...
          it 'does not save the new document' do
            expect(child_role).not_to be_persisted
          end
          ...
          it 'sets the association metadata on the child' do
            expect(child_role._association).not_to be_nil
          end
  ...
  describe '#concat' do
    ...
    context 'when concatenating an empty array' do
      ...
      before do
        expect(person.addresses).not_to receive(:batch_insert)
  ...
  context 'when destroying an embedded document' do
    ...
    it 'removes the document from the unscoped' do
      expect(person.addresses.send(:_unscoped)).not_to include(address_one)
    end
    ...
    context 'when the model does not have an updated_at column' do
      ...
      shared_examples_for 'a cache_version generator' do
        it 'produces a trivial cache_version' do
          expect(pages.cache_version).to eq "#{pages.length}"
        end
      end
