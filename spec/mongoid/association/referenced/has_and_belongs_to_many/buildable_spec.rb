...
describe Mongoid::Association::Referenced::HasAndBelongsToMany::Buildable do
  ...
  context 'when the object is already related to another object' do
    ...
    context "when using <<" do
      ...
      before do
        person1.user_accounts << user_account
        person2.user_accounts << user_account
      end

      it 'clears the object of its previous relation' do
        expect(person1.user_accounts).to eq([])
        expect(person2.user_accounts).to eq([user_account])
      end
    end
    ...
    context "when using concat" do
      ...
      before do
        person1.user_accounts.concat([user_account])
        person2.user_accounts.concat([user_account])
      end
