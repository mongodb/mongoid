...
describe Mongoid::Persistable::Deletable do
  describe '#delete' do
    ...
    context 'when deleting all documents with a custom discriminator key' do
      ...
      let(:firefox) { Firefox.create!(name: 'firefox') }
      let(:firefox2) { Firefox.create!(name: 'firefox 2') }
      let(:browser) { Browser.create!(name: 'browser') }
      let(:canvas) { Canvas.create!(name: 'canvas') }
