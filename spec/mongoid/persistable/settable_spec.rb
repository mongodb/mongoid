  ...
  context 'when the field is already set locally' do
    ...
    context 'when the field is a nested hash' do
      ...
      context 'when a leaf value in the nested hash is updated to a number' do
        ...
        before do
          church.set('location.address.city' => 12_345)
        end

        it 'updates the nested value to the correct value' do
          expect(church.name).to eq('Church1')
          expect(church.location).to eql({ 'address' => { 'city' => 12_345, 'street' => 'Yorckstr' } })
        end
      end
      ...
      context 'when the nested hash is many levels deep' do
        ...
        context 'setting value to a string' do
          it 'keeps peer attributes of the nested hash' do
            ...
            expect(church.name).to eq('Church1')
            expect(church.location).to eql({ 'address' => { 'state' => { 'address' => { 'city' => 'Munich',
                                                                                        'street' => 'Yorckstr' } } } })
          end
        ...
        context 'setting value to a hash' do
          it 'keeps peer attributes of the nested hash' do
            ...
            expect(church.name).to eq('Church1')
            expect(church.location).to eql({ 'address' => { 'state' => { 'address' => { 'city' => { 'hello' => 'world' },
                                                                                        'street' => 'Yorckstr' } } } })
          end
      ...
      it 'raises AttributeNotLoaded' do
        lambda do
          agent.set(title: '008')
                               ...
                               /Attempted to access attribute 'title' on Agent which was not loaded/)
      ...
      it 'raises AttributeNotLoaded' do
        lambda do
          agent.set(number: '008')
                               ...
                               /Attempted to access attribute 'number' on Agent which was not loaded/)
